...
def etl_wcs(wcs_fn, filtered_date_df, web_page_df, cpu=False):
    if cpu:
...
def get_session_id_from_session_boundary(session_change_df, last_session_len, cpu=False):
    """
...
def get_session_id(df, cpu=False):
    """
...
def get_sessions(df, cpu=False):
    df = df.sort_values(
        by=["wcs_user_sk", "tstamp_inSec", "wcs_sales_sk", "wp_type_codes"]
    ).reset_index(drop=True)
    df["session_id"] = get_session_id(df, cpu=cpu)
    return df
...
def reduction_function(df, REVIEW_CAT_CODE, cpu=False):
    if cpu:
        import pandas as cudf
    else:
        import cudf
...
def main(client, config):
    ...
    codes_min_signed_type = np.min_scalar_type(-len(cpu_categories))
    ...
    task_ls = [
        delayed(etl_wcs)(
            fn, filtered_date_df.to_delayed()[0], web_page_df.to_delayed()[0], cpu=config["dask_cpu"]
        )
